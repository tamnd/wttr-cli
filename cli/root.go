// Package cli assembles the wttr command tree from the wttr
...
// NewApp assembles the kit application from the wttr domain. The
...
//
// To add a command, declare it in wttr/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := wttr.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(wttr.Domain{}).Register(app)
