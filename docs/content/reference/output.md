...
describes what users get. Pick a format with `-o`, or let wttr choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
wttr <command> -o table   # aligned columns for reading
wttr <command> -o jsonl   # one JSON object per line, for piping
wttr <command> -o json    # a single JSON array
wttr <command> -o csv     # spreadsheet friendly
wttr <command> -o tsv     # tab-separated
wttr <command> -o url     # just the URL column
wttr <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
wttr <command> --fields id,title,url
```
...
```bash
wttr <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
wttr <command>            # a table, because this is a terminal
wttr <command> | wc -l    # JSONL, because this is a pipe
```
